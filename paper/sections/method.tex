...
We formulate the problem in the classic few-shot mold. A task is a
distribution $P_\mathcal{T}$ over queries, revealed only through a
\emph{support set} $S = \{q_1, \dots, q_k\} \sim P_\mathcal{T}$ of $k$
example queries --- the deployer's sample of the intended workload, and the
\emph{only} task-specific information any method receives. Alongside the
support set, every method is given the same three resources: (i) a single
teacher agent $\pi_T$ that acts by emitting executable code and is accessed
strictly as a black box --- an API that returns sampled trajectories and
charges per emitted token, with weights and logits out of reach, matching
the regime where the strongest teachers are closed commercial models and
ruling out distribution-matching distillation \citep{gu2024minillm,
agarwal2024gkd} from the start; (ii) an open-weight base student $\theta_0$,
trainable locally; and (iii) a token budget $b$ that caps the total number
of teacher tokens the method may purchase over its entire run. Nothing else
is provided: there is no pre-existing trajectory pool and no auxiliary
labeled corpus --- every demonstration a method trains on must be generated
by querying $\pi_T$, and every returned token is charged against $b$.
Prompts sent to the teacher are constructed by the method itself (solving
the support queries, sampling variants of them, probing); the support set
is never expanded.

From $(S, \pi_T, \theta_0, b)$ each method must produce: (1) a boundary
...
$(R_{\mathrm{cov}}, L)$ under matched $(S, \pi_T, b)$, the
budget-to-target level $B^{*}(\tau)$, and the fraction of purchased tokens
that are excluded from training because the returned generation failed
verification --- wasted budget, an audit of how well each method's spending
is aimed. Baselines run under exactly this protocol: a selection method
that presupposes a corpus is given the same pay-per-token teacher and must
generate its own.

The method relies on two assumptions. \textbf{A1 (white-box student):} the
student's gradients are accessible; this is all that gradient features and
atoms consume. \textbf{A2 (success signal):} some check decides whether an
attempt succeeded; certificates consume only this binary outcome, so an
execution verifier, a unit test, or a judge are interchangeable.
Code-acting agents are the instantiation we evaluate, not a requirement.
