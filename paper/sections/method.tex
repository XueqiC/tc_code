...
\subsection{Distillation: weighted imitation with behavioral timing}
\label{sec:distill}
The purchased demonstrations are distilled into the student by supervised
fine-tuning, with two departures from uniform behavior cloning, both
driven by the demand estimate. First, examples are not weighted equally:
each admitted demonstration's loss is scaled by how much of the remaining
demand its code serves, so a demonstration that mostly repeats an
already-satisfied skill contributes less than one that carries scarce
skills --- importance weighting with weights read off the same accounting
that directed acquisition. In multi-turn environments the weight is
applied per turn, the granularity at which the environment responds
\citep{liu2025structured}. Second, the student's own failures direct a
corrective phase in the style of \textsc{DAgger} \citep{ross2011dagger}:
the student is rolled out locally on held-out support queries, the
execution check localizes the states where it goes wrong, and those very
states are sent to the teacher as continuation prompts --- charged
against $b$ like any other purchase --- yielding demonstrations of
recovery behavior precisely where successful teacher trajectories are
silent. Throughout training, gradient geometry is never asked to judge
success: three pre-registered experiments show it cannot (instance-level
gradient signals fail to predict success; re-selecting against residual
gradients selects noise; geometric absorption bottoms out two orders of
magnitude before behavioral competence forms,
Section~\ref{sec:mechanism}). All judging is behavioral: a probe every
$8$ steps measures the fraction of held-out calibration queries yielding
runnable solutions, and the best-scoring checkpoint is kept, making
aggressive small-budget training safe because drift past the competence
peak is detected and rolled back.
