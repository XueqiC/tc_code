...
\caption{The pipeline. Colored tiles are capability atoms: blue tiles are
skills the task needs, orange tiles are out-of-scope skills, gray tiles are
inactive. (1)~Each of the $k$ example queries is read through one backward
pass at the student's initialization; the gradient names the skills that
student is missing. (2)~A sparse dictionary over many teacher traces yields
a library of skill atoms, auditable on data (one atom's top activations are
exactly \texttt{JOIN} clauses); the specification is the set of atoms the
$k$ queries need. (3)~Teacher traces are scored by the atoms they supply:
traces covering the specification fill the token budget, traces carrying
out-of-scope atoms are penalized by $\lambda$ and rejected, and training
stops once the remaining in-specification demand is absorbed. (4)~At
deployment a conformal gate routes requests: its threshold is the
calibration quantile, so coverage is guaranteed rather than tuned, and both
sides of the match carry certificates (in-task lower bound $0.93$, off-task
upper bound $0.25$; Section~\ref{sec:results}).}
\label{fig:pipeline}
\end{figure}
