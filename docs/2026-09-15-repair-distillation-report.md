...
1. **预注册判读:第 4 行——"B 在登记的检查下没有稳定收益"**(分析器自动判读;等价边界 ±2 pp,要求整个配对 95% CI 落入)。
   **措辞修正(用户 21:28)**:B−对照 [−1.4, +2.6]、B−A [−1.5, +3.6]、B−C [−2.4, +2.0] 都不能说明等价于零——准确结论是"未检出稳定优势,也未建立等价";
   按预注册资源规则停止本配置合理,但不能写成"学生思路条件化被否定"。"B 必须超过 C"也不是该假设成立的必要条件。
...
**修正**:区间宽于边界,等价未建立;结论限定为"本配置未检出稳定优势",机制未被否定。另一个混杂:动作 token 占监督 token 的比例 A ≈ 32%、B ≈ 85%、C ≈ 66%,
B 相对 A 同时改变了动作条件与动作项的相对权重(并取消了教师 Thought 监督),小幅正向不能归因于条件化。下一轮 A*/B* 设计消除这一混杂(见 §12)。
...
## 12. 用户 21:28 的决定与下一轮(A*/B*)

保留 U0 主干;暂停 C 的修正续写;对"学生思路条件化"做最后一次**材料有教学差异、动作监督强度严格一致**的检验。
A*:学 s→r^T 与 (s,r^T)→a^T;B*:学 s→r^T 与 (s,r^S)→a^T;动作 / 终止标记 / 出现次数 / 归一化分母与系数完全相同,学生 Thought 只作输入且不进分母。
材料:真实 U0 状态;教师**先独立**给出计划与动作(不展示学生拟议动作);随后按差异提名并分类(等价查询 / 合法替代 / 重复 / 错误实体 / 过早结束 / 漏结束),
保留"应继续"与"可结束"两类,按父任务限制重复;记录 d_i = ℓ(a^T|s,r^S) − ℓ(a^T|s,r^T) 与绝对 NLL 作描述与分层,不作权重。
规模:A*/B* × 两配对种子 + 同批 U0 对照;剂量按新银行曝光定(3 遍规则,不复制 113 步);局部检查含自主生成、正确/错误结束、继续执行后的结果,按父任务聚合;
dev-500 作开发证据,确认用未参与选择的父任务。主比较 B*−A*;四行判读:两臂都改善且接近 → 材料有效、前缀未证明额外价值;B* 稳定优于 A* 且完整任务同向 → 前缀有增量;
仅局部改善 → 局部适应;两者均无 → 停止投入该条件化机制。创新边界:SCoRe 已有"学生执行、教师纠正、再蒸馏";候选贡献 = 有限教师预算下识别标准教师前缀掩盖的动作学习缺口,
并用保持正确推理监督的学生前缀训练提高迁移。
