...
## Failure analysis 与跨模态桥(PI 要求 2026-08-13 下午)
主因 F1:技能住在响应空间,账开在 prompt 空间 —— prompt 指纹编码题目
表面/主题而非解题技能(atoms 的 0.906 可辨识性证据全在响应/轨迹侧);
需求退化为主题分布 -> 买得对题不保证对技能;解释"纯度碾压、in-task
平庸"的分裂。
对策:跨模态桥 —— 种子采购给出 (prompt指纹, 响应指纹) 配对,学
prompt码->响应技能码 映射(线性回归);需求 = spec 解答的响应侧技能码
(隐式需求内置),供给预测 = 桥(候选prompt码)。桥只用已购数据,合法;
每轮更准(获取函数的主动学习)。
次因:F2 批内不扣账->序贯扣;F3 码单位汇率->已购每token码强度在线
校准;F5 权重方差->首epoch平权+按语料规模收紧clip;F6 探针10条->15条
平滑;F4 0.8B指纹信噪比 = scaling 适用范围(目标学生2B-9B)。
顺序:F2/F3/F6 小刀先上;F1 桥 ~80行;F5 同批。
