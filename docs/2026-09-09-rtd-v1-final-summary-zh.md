...
单种子(seed 0)。六个 run = 三个方法臂(R0 随机采购+固定 a=.5;R1 完整 RTD;R1s 学习标量门控)× 两台机器(hpg B200;rai 三种卡),**不是**多种子重复。base 学生 46.74;单种子噪声带 ±1.3。完成度:hpg 三臂三轮全部完成;rai R1s 完成;rai R1 第 2 轮评测进行中;rai R0 第 3 轮等 GPU。未完成格子标"未完成",出来后补发。
...
3. **反馈更新有副作用。** 温度 1 采样的反馈 rollout 中格式错误动作逐轮上升:hpg R0 0/18/33、R1 0/12/24、R1s 46/7/36,rai R1s 0/10/40——随机臂同样上升,说明是反馈更新(REINFORCE 步)把采样分布推松,与采购无关;greedy 官方分尚未受影响。
...
3. **格式错误率作为每轮健康指标**:每轮上报温度 1 采样的解析失败率;若继续逐轮上升,把反馈步的 KL 上限作为限制项(先观察,不先干预)。
