...
## 0. 用户 14:17 CDT 的修正(以此为准,下文原表述保留作记录)

- **按"确实可交还"的同一批任务重读**:k1 可交还 19 题:U0 自行 2/19 → 交还后学生完成 2/19(新增 intermission,丢掉 Peter Brown);k3 可交还 14 题:1/14 → 2/14(新增 Osawatomie)。
  两种交还合计,新增恢复案例只有 **2 个父任务**。k3 的 CI 排除零支持的是"教师参与的混合执行系统更好",尚未支持"连续三步教学能让学生独立执行得更好"。
  19 / 14 是由教师是否提前终止决定的条件子集,只用于解释结果构成,不是新的总体效果估计。
- **四个证据型案例**:SNL 与 Peter Brown 在本轮 k0 已经成功;整段教师前缀同时传递了 Thought、动作和 observation,不能把效果单独归因于新证据;即时接管成功没有回答
  "学生训练后能否在新任务上自主产生这些检索行为"。原文"教师的可迁移优势 = 检索交互"改为:**在四个教师取得新证据的案例中,观察到了学生接续完成任务;其中两个父任务相对本轮 U0
  参照获得新增成功;相应检索行为的训练迁移价值尚未检验。**"交还需要到决定性检索步为止"只作案例观察,最佳教学长度未确定(只比较了两个交还位置)。
- **非证据型成功**:8 个里 1 个明确来自学生已有上下文,3 个是部分证据;对缺少完整证据支持的成功,只能说**本次工具记录未完整解释答案来源**,不能确定教师内部依赖了什么。
- **资源决策**:这一周暂停 HotpotQA 新机制的训练与扩展,不新增恢复片段训练、不追加教师提示搜索;理由是**当前证据与剩余时间不匹配**,不是"HotpotQA 上交互蒸馏没有空间"
  (教师本轮也常未取得足够证据,不是教师能力上限)。轨迹、账本、报告归档;资源转向旧 RTD 核心增量复核。
