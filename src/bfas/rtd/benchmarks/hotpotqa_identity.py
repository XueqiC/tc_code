...
def evaluation_harness_identity(root, config):
    ...
    names = ['src/bfas/adapters/hotpotqa.py', 'src/bfas/hotpotqa.py', 'tools/hotpotqa_eval.py',
        'src/bfas/run.py', 'src/bfas/rtd/benchmarks/webshop_evaluation.py',
    ...
    snapshots = {p.name: file_hash(p) for p in sorted(cache.glob('*.json'))}
    return dict(version='rtd-hotpotqa-adapter-scoring-v1', benchmark='hotpotqa',
        ...
        environment=dict(wiki_version=hp.WIKI_VERSION, snapshots=snapshots, offline=True),
