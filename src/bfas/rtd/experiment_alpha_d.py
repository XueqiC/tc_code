"""Rev 3 fixed window order on the merged executor, with durable source pairs.

Acquisition consumes its historical posterior before any current-step return
rollout. Its later additive insertion labels have a separate reference/role.
...
class AlphaDExperimentMixin:
    ...
    def alpha_draw_pairs(self, records, *, role):
        ...
        for i, record in enumerate(records):
            ...
            if before == after or any(src is old for src in sources for old in cached):
            ...
            self.journal.append('alpha_d_source_pair', round=s['round'], step=s['step'], role=role,
                ...
                rng_before=before, rng_after=after, cache_reused=False)
        return tuple(pairs)

    def alpha_freeze(self, records, *, role, fixed=False):
        s = self.state
    ...
    def alpha_step_start(self):
        ...
        s.update(decision=s['step'] in (1, 4, 7, 10), selected=[], pending_ids=[], labels={},
            ...
            owned_before=tuple(s['owned']), old_noop=False)
        if s['decision']:
    ...
    def alpha_revealed(self):
        ...
        old = [r for p in self.packages() for r in self.supervision_records(p)]
        if not old and len(records) < self.slots and self.slots == 40:
            raise ValueError('full exposure requires an already-paid inner old pool to fill E=40; '
                             'source-only placeholders are not exposure units. Supply bootstrap evidence '
                             'or explicitly configure fewer states as random exposure')
        if not old and not records:
            raise ValueError('no purchased supervision records for an exposure unit')
        while old and len(records) < self.slots:
            records.append(old[int(self.rng.integers(len(old)))])
        s['alpha_chi'], alpha, weights = self.alpha_freeze(records, role='commit')
        ...
        self.journal.append('alpha_d_reference', round=s['round'], step=s['step'],
            role='source_selection_theta_S_0', parameter_hash=tensor_state_hash(ref.theta0),
            start_hash=ref.start_hash, same_batch=True, return_objective=RETURN_OBJECTIVE)
        if s['decision']:
            s['feedback_tasks'] = self.choose_feedback_tasks()
            scores = []
            feedback = self.feedback(ref.theta0, 'source_selection_feedback', trajectory_scores=scores)
            if feedback.parameter_hash != tensor_state_hash(ref.theta0):
                raise ValueError('source-selection feedback must be at this batch theta_S(0)')
            rollouts = s['feedback_rollouts']['source_selection_feedback']
        ...
        s['alpha_d_control'] = dict(z_hat=z.tolist(), epsilon_hat=[float(x) if np.isfinite(x) else None for x in error],
            ...
            reference_hash=tensor_state_hash(ref.theta0), actual_hash=tensor_state_hash(s['actual']))
    ...
    def alpha_acquisition_labels(self):
        """D2 additive surrogate, measured AFTER choice; distinct from d's J.

        This historical acquisition point is not theta_S(0). D9 owns replacing
        this approximation with full-update set/LOO values including teacher mass.
        """
        s, ref = self.state, self.state['d_reference']
        old_indices = [i for i, p in enumerate(s['alpha_pairs']) if not p.record.is_new]
        old = {n: sum((ref.baseline_gradients[i][n].to(p)/len(old_indices) for i in old_indices),
                      torch.zeros_like(p)) for n, p in s['alpha_start'].items()}
        acquisition = InsertionReference(s['alpha_start'], None, s['step_rule'], old_slots=len(s['alpha_pairs']),
            exposure_slots=len(s['alpha_pairs']), old_gradient=old, exact_noop=all(not g.any() for g in old.values()))
        s['reference'] = acquisition
        ...
        if gs:
            feedback = self.feedback(acquisition.updated, 'acquisition_surrogate_feedback')
            stats = self.paid_statistics(gs, 'acquisition_surrogate_feedback')
        ...
        self.journal.append('alpha_d_acquisition_reference', round=s['round'], step=s['step'],
            role='historical_additive_insertion_surrogate', parameter_hash=acquisition.reference_hash,
            source_selection_reference_hash=tensor_state_hash(ref.theta0), return_objective=RETURN_OBJECTIVE,
            feedback_role='acquisition_surrogate_feedback' if gs else None,
    ...
    def alpha_actual(self):
        ...
        if s['decision']:
            ...
            if feedback.parameter_hash != tensor_state_hash(s['parameters']):
    ...
    def alpha_feedback_commit(self):
        ...
        row = dict(round=s['round'], step=s['step'], decision=s['decision'], inner_fold=(s['round']-1)%2,
            acquisition_protocol='alpha_d_historical_additive_surrogate', selected=list(s['selected']), slots=len(pairs),
            max_new_packages=self.max_new_packages, fixed_evidence=self.fixed, start_hash=ref.start_hash,
            reference_hash=tensor_state_hash(ref.theta0), actual_hash=tensor_state_hash(s['parameters']),
            exact_noop=ref.start_hash == tensor_state_hash(s['parameters']), audit_passed=s['audit_passed'],
            raw_old_slots=len(old), raw_new_slots=len(new), weighted_new_slots=len(new), weighted_old_slots=len(old),
            old_coefficient=len(old)/len(pairs), position_share=1/len(pairs),
            exposure_mode='full exposure' if self.slots == 40 else 'random exposure',
            exposure_units=sum(p.record.teacher is not None for p in pairs),
