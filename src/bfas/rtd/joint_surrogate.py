...
from .alpha_d import calibrate_d, dot, gram, gram_statistics, solve_d, target_weights
from .functional_step import snapshot

ACQUISITION_SURROGATE = 'acquisition_surrogate_full_update_with_teacher_injection'


def objective(z, error, K, d, d_lambda, *, d_lambda_normalisation='mean_diagonal', calibration=None):
...
def control(z, error, K, alpha, *, mode='joint', d_lambda=1., d_lambda_normalisation='mean_diagonal',
            calibration=None, zero_reason=None,
    ...
    units = calibration.journal(d_lambda)
    ...
    if zero_reason:
        ...
        solver = dict(converged=None, iterations=0, reason=zero_reason, **units)
        ...
        objective_options = dict(d_lambda_normalisation=d_lambda_normalisation, calibration=calibration)
        options = dict(d_lambda=d_lambda, tolerance=tolerance, max_iterations=max_iterations, **objective_options)
        ...
        joint_gain = objective(z, error, K, joint, d_lambda, **objective_options)
        independent_gain = objective(z, error, K, independent, d_lambda, **objective_options)
        additive = objective(z, error, np.diag(np.diag(K)), independent, d_lambda, **objective_options)
    ...
    return d, dict(controller=mode, d_star=d.tolist(), joint_d=joint.tolist(), independent_d=independent.tolist(),
        ...
        source_teacher_shares=shares, K=K.tolist(),
        K_statistics=gram_statistics(K/calibration.scale, redundancy_threshold) | dict(
            raw_trace=float(np.trace(K)), raw_trace_scientific=format(float(np.trace(K)), '.17e'),
            units=units['objective_units']),
        z_hat_normalised=(z/calibration.linear_scale).tolist(),
        epsilon_hat_normalised=[float(x) if np.isfinite(x) else None for x in error/calibration.linear_scale],
        ...
        optimized_variables='d_only', weights_and_alpha_frozen=True, raw_z_prefilter=False, **units)


def set_solution(baseline, directions, weights, alpha, start, reference, step, feedback,
                 statistic, *, d_lambda=1., d_lambda_normalisation='mean_diagonal', calibration=None,
                 zero=False, error_mode='loo', mode='joint',
    ...
    g^T v/sqrt(s) - lambda/s * delta0^T H v and a constant baseline value. When reference
    ...
    # control divides linear terms by sqrt(s); apply the cross term in the
    # equivalent original units so it is normalised exactly once.
    effective_z = z-(d_lambda/calibration.linear_scale)*cross
    d, diagnostic = control(effective_z, error, K, alpha, mode=mode, d_lambda=d_lambda,
        d_lambda_normalisation=d_lambda_normalisation, calibration=calibration,
    ...
    penalty = float(error @ np.abs(d))/calibration.linear_scale if np.isfinite(error).all() else 0.
    value = dot(feedback.gradient, update)/calibration.linear_scale-d_lambda*cost/(2*calibration.scale)-penalty
    diagnostic.update(value=value, baseline_linear_gain=dot(feedback.gradient, delta)/calibration.linear_scale,
        full_update_quadratic_cost=d_lambda*cost/(2*calibration.scale), uncertainty_penalty=penalty,
...
def validate_pair(kind, query_id, first, d1, second, d2, prediction, start, step, evaluate, *,
                  check_updates=None, prediction_scale=1.):
    ...
    return dict(comparison=kind, query_id=query_id, prediction=prediction*prediction_scale,
        surrogate_prediction=prediction, prediction_scale=prediction_scale, prediction_units='original',
        realised_paired_gain=realised, realised_minus_predicted=realised-prediction*prediction_scale, full=a, control=b,
...
def marginal_values(pairs, batch, old, start, reference, step, feedback, statistic, *, revealed_ids, **options):
    ...
    if len(old.baseline_gradients) != len(pairs):
        raise ValueError('one predetermined old-pool replacement per unit required')
    # Deletion must not redefine the units of a marginal value. Calibrate on
    # the full set once (or reuse the actual controller's window calibration).
    if options.get('calibration') is None:
        options['calibration'] = calibrate_d(gram(batch.directions, step.diagonal),
                                             options.get('d_lambda_normalisation', 'mean_diagonal'))
    def value(candidate):
    ...
    return values, dict(full_set_value=full, marginal_values=values,
        ...
        **options['calibration'].journal(options.get('d_lambda', 1.)))
