...
class BatchExperimentMixin:
    ...
    def remeasure_owned(self):
        ...
        for q in ids:
            ...
            with self.scope('drift_package_gradient'):
                gs[q] = self.gradient(targets, chi, s['reference'].start)
    ...
    def batch_gate_vjp(self, targets, chi, feedback):
        """D1 hook: must differentiate the SAME operator as self.gradient()."""
        s = self.state
        return streamed_gate_vjp(targets, chi, s['phi'], self.backend, s['reference'].start,
                                 s['step_rule'], feedback, gate=self.gate)
        ...
        if s['decision']:
            ...
            if self.gate in {'linear_sigmoid', 'scalar_sigmoid'}:
                with self.scope('gate_vjp'):
                    ...
                    if not s['old_noop']:
                        vjp = (1-len(s['selected'])/self.slots)*self.batch_gate_vjp(s['old_targets'], s['old_chi'], feedback)
                    for q in s['selected']:
                        vjp += self.batch_gate_vjp(s['batch_targets'][q], s['batch_chi'][q], feedback)/self.slots
