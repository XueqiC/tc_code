...
class AcquisitionStopped(RuntimeError):
    """A caller's hard budget forbids another purchase; no attempt was made."""
...
def demo_payload(demo: Demo) -> dict[str, Any]:
    ...
    if isinstance(demo.raw, Mapping) and "teacher_commands" in demo.raw:
        ...
        if "student_prefix_commands" in demo.raw:
            payload["student_prefix_commands"] = _json_safe(demo.raw["student_prefix_commands"])
    return payload
...
def _demo_from_payload(task_id: str, value: Any, record: Mapping[str, Any]) -> Demo:
    ...
    return Demo(
        ...
        raw={
            ...
            **({"student_prefix_commands": value["student_prefix_commands"]}
               if "student_prefix_commands" in value else {}),
...
def _teacher_name(benchmark: str | Path, adapter: BenchmarkAdapter) -> str:
    ...
    if str(benchmark) == "bfcl":
        return os.environ.get("BFAS_BFCL_TEACHER", os.environ.get("BFAS_TEACHER", "gpt-5.4"))
