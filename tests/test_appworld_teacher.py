...
@pytest.mark.parametrize('name,key', [('openai/gpt-5.6-luna', 'max_completion_tokens'),
                                     ('openrouter/openai/gpt-5.4', 'max_tokens')])
def test_teacher_request_specific_completion_cap(monkeypatch, name, key):
    resolved = config(monkeypatch, name)
    calls = stub_client(monkeypatch, [payload()])
    teacher.generate_reply(resolved, [{'role': 'user', 'content': 'look'}],
                           max_completion_tokens=7, retries=0)
    assert json.loads(calls[0][0].data)[key] == 7
    assert teacher.MAX_COMPLETION_TOKENS == 2048


@pytest.mark.parametrize('failure', [http_error(429), http_error(503), socket.timeout()])
def test_teacher_bounded_collection_disables_implicit_retries(monkeypatch, failure):
    resolved = config(monkeypatch, 'openai/gpt-5.6-luna')
    calls = stub_client(monkeypatch, [failure, payload()])
    monkeypatch.setattr(teacher.time, 'sleep', lambda _: pytest.fail('unexpected retry'))
    with pytest.raises(teacher.TeacherAPIError):
        teacher.generate_reply(resolved, [{'role': 'user', 'content': 'look'}],
                               max_completion_tokens=7, retries=0)
    assert len(calls) == 1
