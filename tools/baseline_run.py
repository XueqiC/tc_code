...
def arguments(argv=None):
    ...
    parser.add_argument("--seed", type=nonnegative_seed, default=0,
                        help="training randomness only; nonzero seeds append _s<seed> to each run directory")
...
def prepare(args):
    ...
    try:
        verify_seed_zero(args.run_dir, manifest)
    except ValueError as error:
...
def train_worker(directory, manifest):
    ...
    try:
        seed_training(manifest["seed"])
        with stage("model_load"):
            backend = load_backend(dict(manifest["config"], training_seed=manifest["seed"] % (2**64)),
                                   manifest, journal)
        trainer = PaperTrainer(backend, [TeacherRow(**r) for r in rows], manifest["config"],
                               manifest["method"], directory, journal, manifest=manifest)
...
def run_budget(args):
    ...
    worker = [sys.executable, str(Path(__file__).resolve()), "--method", args.method,
        ...
        "--seed", str(args.seed), "--run-dir", str(args.run_dir)]
    if args.smoke:
        worker.append("--smoke")
    try:
...
def main(argv=None):
    args = arguments(argv)
    # A repeat is a sibling of the supplied seed-zero name, even if that name
    # is a symlink. Do not redirect its output through the reference directory.
    args.run_dir = (args.run_dir.parent.resolve()/args.run_dir.name
                    if args.seed and not args._phase else args.run_dir.resolve())
    ...
    if args._phase:
        ...
        if args.seed != manifest["seed"] or args.seed != manifest["config"]["training_seed"]:
            raise ValueError("worker seed differs from prepared training seed")
    ...
    from bfas.rtd.baselines.paper_seeds import seeded_directory
    runs = []
    for cap in levels if levels is not None else [None]:
        ...
        directory = seeded_directory(directory, args.seed)
