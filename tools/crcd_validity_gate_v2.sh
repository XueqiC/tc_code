...
set -uo pipefail
cd "$(dirname "$0")/.."
GATE=${GATE_DIR:-data/bfcl_sft/gate}
...
serve() {  # $1 model path
  VLLM_USE_FLASHINFER_SAMPLER=0 nohup envs/vllm-serve/.venv/bin/vllm serve "$1" --served-model-name Qwen/Qwen3.5-4B Qwen/Qwen3.5-4B-FC \
    --port $PORT --gpu-memory-utilization ${GPU_UTIL:-0.4} --max-model-len 32768 --max-num-seqs 256 > logs/vllm_gate${GATE_TAG:-}.log 2>&1 &
  VPID=$!
...
measure() {  # $1 rates file
  PYTHONPATH=src:tools envs/bfcl/.venv/bin/python tools/bfcl_event_mine_single.py --port $PORT --k ${GATE_K:-4} --no-support \
    --gen data/bfcl_sft/gen_pool_v3.jsonl data/bfcl_sft/gen_pool_v4.jsonl data/bfcl_sft/gen_oos.jsonl --irrelevance \
    $( [ -f $GATE/measure_ids.json ] && echo --only-ids $GATE/measure_ids.json ) --rates-out "$1" --rates-key ${RATES_KEY:-id} --out /dev/null 2>&1 | grep -E 'rates|FINAL' | cut -c1-160
}
# support single-turn tasks are part of the measurement set too: include them by not passing --no-support
measure_all() {
  PYTHONPATH=src:tools envs/bfcl/.venv/bin/python tools/bfcl_event_mine_single.py --port $PORT --k ${GATE_K:-4} \
    --gen data/bfcl_sft/gen_pool_v3.jsonl data/bfcl_sft/gen_pool_v4.jsonl data/bfcl_sft/gen_oos.jsonl --irrelevance \
    $( [ -f $GATE/measure_ids.json ] && echo --only-ids $GATE/measure_ids.json ) --rates-out "$1" --rates-key ${RATES_KEY:-id} --out /dev/null 2>&1 | grep -E 'rates|FINAL' | cut -c1-160
}
...
for POOL in $GATE/pool_c*.jsonl; do
  ...
  TAG=crcdgate${GATE_TAG:-}_${C}_s0
  [ -f $GATE/rates_${C}.json ] && { echo "[gate] $C already measured"; continue; }
  echo "[gate] === train $TAG on $(wc -l < $POOL) events ==="
  env AW_POOL_PATH=$PWD/$POOL AW_DISTILL=ddpo AW_SFT_SKIP=1 AW_DDPO_EPOCHS=${GATE_EPOCHS:-1} .venv/bin/python src/appworld_train.py \
