...
def main() -> int:
    ...
    sql_rows, _ = split("sql")
    join_rows = [
        r for r in sql_rows if "join" in (r["prompt"] + r["response"]).casefold()
    ]
    other_sql = [
        r for r in sql_rows if "join" not in (r["prompt"] + r["response"]).casefold()
    ]
    gsm = gsm_rows[:60]
    joins = join_rows[:60]
    print(f"[coupling] gsm={len(gsm)} sqljoin={len(joins)} "
          f"(other sql pool {len(other_sql)})", flush=True)
