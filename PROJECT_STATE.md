...
### 待用户决定(按我建议的优先级)
1. **合法动作约束解码验证**(零训练,拿现有 39 步 checkpoint 直接评;若 23 → 55+,机制与方法一起坐实)——**这是目前唯一由机制推出、未被证伪的方法候选**;
2. 评测规模:HotpotQA 扩到 2,000 题(代码在 worktree `tc-alignment-evalext` / 分支 `hpqa-eval-ext`,`9fa17ef8`,已测**未上线**;
   卡在 `paper_evaluation.py` 四处硬编码 500);
3. 决策掩码臂(**必须与 2 同时上**,否则 500 题 MDE=3.9 判不出来);
4. Kang 那一行选 A / B / C;5. 2% 档差的 61 题补不补(Azure 两把 key 均 200,但共享窗口读数疑似静态,采购前以真实响应头为准)。
