...
- 03:54 CDT **HotpotQA 904×3 遍格(1031346)按 6 小时上限必然超时,已提前处置**。它在 4h41 时只走到 91/156 步(≈2.8 min/步),剩 1h19 墙钟却还需 ≈3.4h; 不存中间 checkpoint、不支持续跑,超时会全丢。
  处置: → 旧 run dir 移入  → 以  重投为 **1031701**(已 R,gpu4/qbd512)。代价是丢掉 4h41,但比再空烧 1h19 后同样全丢要好。
  **规则(与 ALFWorld 同源,现推广)**:任何 >100 步的格都不能用 slurm 脚本里  的默认值;ALFWorld ≈2.7 min/步、HotpotQA ≈2.8 min/步,提交时按  估并留一倍余量,用  覆盖。
