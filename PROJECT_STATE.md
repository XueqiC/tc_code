...
- P2,luna,attempt 6–9,T=0.7,每任务最多 2 条:41 次尝试,**26 验证(63%),扫描过滤去掉 7 → 可用 19 条(heat 7 / cool 6 / two 6)**;1,050 次调用,结算 864,978 token。
  ...
  mike 提交 d0targeted_v2 s0/1/2 = 861755/56/57;评测循环含 TARG 并已重启。
- 这是框架"按子技能缺口追加"步的第一次实测:cool/heat/two 三类各加 ≤2 条示范。

### 忠实版 SmartAD 三 seed:88 / 89 / 82 = 86.3(61.7%)vs mike base 81.3 → +5.0 pp(2026-09-23 02:17 CDT)
- 今晚最强的臂,而且是**基线**(SmartAD 按论文口径修正后)。方法要赢的门槛 = 86 左右而不是 base 81。配对细节见表 §Y 续。
- 与原版 SmartAD(token 均值选例、除 token 数归一)的差:原版 mike s2 = 84、LONI s0/s1 = 82/82(跨平台);选例统计与归一的修正带来 +2~+5(待同平台三 seed 对照)。

### 忠实版 SmartAD(turn_mean 选例 + 权重和归一)s0 pass-10 = 88,目前最高单格(2026-09-23 02:02 CDT)
- 对 mike base78 **+7.14**(28 题不一致 9/19,**p=0.087**,今晚最接近显著的一格);对 base84 +2.86。按类型 clean 20/15、two 16/11、**cool 8/8(不掉)**、heat 5/8。
  base 必输题赢 10/47,**必赢题只丢 6/70(所有格里最少)**。一个 seed;s1/s2 评测中。ADDALL 三 seed 75/83/80 = 79.3。
- 含义:审计修法后的 SmartAD 是一个更强的基线——如果 s1/s2 也在 85+,"方法必须赢它"的门槛就更高;同时它印证了"选材料"是杠杆(它只改了选例统计和归一)。
- GPU4 链 #5(pid 3336118):链 #4 后在 rai 交叉核对 SMWN s0 p10 与 ADDALL s1。表 §Y。

### 01:44 快照:定向采购 19/30 验证(12 任务)、ADD 臂高端点、链位置(2026-09-23 01:44 CDT)
- **定向采购(P2)**:774 次调用、结算 625,226 token,30 次尝试 / 12 任务,**19 条验证**(63%,远高于 REPAIR 的 41%——从任务起点买比从卡住状态救容易)。
- ADD 臂高端点(mike):ADDALL s0/s1 = 75 / 83;ADD1 s1/s2 = 76 / 76(mike base 78/82/84)。SMORIG s2 p10 = 84。
- rai:GPU1 1alt s0 46/184;GPU3 le50tok s2 56/185;GPU4 C s0 收尾/链 #4 待起。

### mike base 三次 78/82/84;按"base 三次都赢/都输/摇摆"拆训练格(2026-09-23 01:30 CDT)
- mike base 均值 81.3(58.1%),跨度 6 题。base 三次都赢 70 题、都输 47 题、摇摆 23 题。
- 训练格在 47 个 base 从不赢的任务上赢 5–10 题(D s2 10、SADTC/SMORIG 9、B 8、LE50 8、C 5–7、ADDALL 6),同时在 70 个 base 必赢的任务上输 6–13 题;
  **净收益 ≈ 0,总分差异主要来自 23 个摇摆题**。→ "学到新能力"与"丢掉旧能力"同量级,这正是类型抵消的另一种表述;方法要赢必须减少"必赢题上的丢失"。
- SMORIG s2 p10 = 84(原版 SmartAD 三 seed:LONI 82/82、mike 84);ADDALL s0 高端点 75。表 §X。

### rai 四格三 seed 快照:材料 +6~8,任务等权在 D0 上 +5、在 87 条上 ≈ +1(2026-09-23 00:40 CDT)
| rai(base 84/81) | D0 30 条 | 87 条 |
|---|---|---|
| 普通 CE | A 74/74/77 = 75.0 | C –/80/83 = 81.5 |
| 任务等权 | **B 77/78/85 = 80.0** | D 83/–/82 = 82.5 |
- B s2 = 85(rai 上唯一 ≥ base 的训练格);B 三 seed +5 于 A;D 对 C 同 seed −1。**两个因素都有效但不可加:材料在 87 条上把 A→C 抬到 base 附近,
  任务等权在小银行上补一部分、在大银行上无额外增益。** 单格都在噪声内(不一致题 19–29),三 seed 均值的方向一致。
- 待 mike 上 A/B 高端点(同平台)与 C s0、D s1 补齐;GPU4 链 #4 正在 rai 评 C s0/D s1/SADTC s0/LE50 s1。表 §T5。
...
- 提交:all87_1alt s2 861675、all87_2per s2 861676(材料量臂第三 seed;seed0 在 rai GPU1、seed1 在 rai GPU3);`mike_base_rep2` 861677。
- mike 队列 10 跑 / 6 排。rai 训出的材料量臂端点由同步循环推到 mike 后也会被自动评(ALT1/PER2)。

### 用户 00:14/00:15:"全部做完汇总成文档;今晚拿到最后方法的雏形和框架;别停,尽量并行"(2026-09-23 00:18 CDT)
- 报告已起草 §9A"方法雏形与框架":方法 = 预算内的示范采购与选择(多轨迹/任务、按类型缺口追加、过滤高扫描轨迹、多样性保留),训练侧普通 CE + 固定更新预算;
  占位符待四格 A/B 高端点、材料量拆分臂、ADD vs REPAIR 数。
- 新并行线:**Codex#20** 给采集器加 `--only-tasks`(按类型缺口定向采购)+ `tools/alf_targeted_tasks.py` + 定向 ADD-DEMO 配方(cool/heat/two 16 任务,attempt 6–9,每任务 2 条);
  好了就用 P2 采(用户:预算随便用)→ D0+定向 并集 → token 端点训练。
- GPU3 链(pid 3227666):le50tok s2 训完后训材料量臂 seed 1(1alt → 2per)。GPU1 链 #9 训 seed 0 并在 rai 评。

### 材料量臂(零采购):87 条银行的 2 条/任务(59 demo)与 1 条/任务替代(30 demo)(2026-09-23 00:15 CDT)
- 目的:把"87 条 = 更多且更多样的示范"拆成材料量与材料身份——`alfworld_k32_all87_2per`(59 包 / 946 回合 / 23,446 token)、
  `alfworld_k32_all87_1alt`(30 包 / 455 回合 / 11,285 token,每任务取候选 id 最大的一条,与 D0 是不同轨迹)。token 端点配置 `pi1_alfworld_k32_all87_{2per,1alt}_tok.yaml`,preflight 过。
- GPU1 链 #9(pid 见 logs/rai_chain_gpu1_material.log):等交叉评测链 #8 结束后训 1alt s0 → 2per s0(各 ~4.75 h)→ 在 rai 评两者高端点。
- rai 交叉核对新数:B s1 t96490 = 78(A 参照 74/74/77 → +4)。
...
- rai(base 84/81):**A 74/74/77(均 75.0)**;B s0 77;**C s2 83;D s0/s2 83/82**;SmartAD 原版 s2 80。
  ...
  mike 上 D(85/85)略高于 C(78/83),待 A/B 高端点与更多 seed 定案。
- GPU1 链 #7 结束;合并模型已删(余 247 GB)。GPU1 空 → 立刻评 B s1/s2、C s1(取消等 C s0 的 GPU4 链 #3 以免重复)。表 §T4。

### mike:B s0 高端点 75(D0 + 任务等权无增益);44 格已提交、17 个高端点训完(2026-09-22 23:30 CDT)
- `mike_Bs0t96490` = **75**(对 base78 −2.1,23 题不一致 13/10,p=0.68;cool 4/8)。→ 任务等权在 D0 上没有带来收益;D 的 85 更像 87 条材料
  (多样性/少重复)与任务等权的合力,拆分要等 mike 的 A 三 seed 与 B s1/s2、以及 rai 上 C s1/s2。
- mike 队列:评测 44 已提交(约 25 完成/在跑);高端点 17 个训完;ADD/REPAIR 臂低端点陆续出(ADD1 s2 68、ADDALL s0 68)。
- rai:GPU1 链 #6/#7(A s2、C s2 评测);GPU4 训 C s0(~02:30)→ 链 #3 评 B s1/s2、C s1。

### rai 盘满(合并模型堆积)→ 删除已评测的合并模型,余 227 GB(2026-09-22 23:25 CDT)
- C s2 的合并因 "No space left on device" 失败(/ 100%,merged_v2 246 GB = 12 个 24 GB 的合并模型);
  删除 10 个已评测完的合并模型(adapter 与评测记录都在,合并可重现),余 227 GB;整机仍 99%(14T),**以后每评完一格就删合并模型**。
- 链 #6(rai_ces2p10)/ #7(重合并 C s2 → rai_Cs2t96490)在 GPU1 跑,监视 bl3xkohwe。

### 跨平台复核:D 在 rai 也比纯 CE 高 8–9 题(2026-09-22 23:20 CDT)
- rai 平台(base 84/81,纯 CE 参照 s0/s1 = 74/74):**D s0 = 83、D s2 = 82**(mike 训的 checkpoint 拉到 rai 评)。
  → "D ≫ 纯 CE"在两个平台都成立(mike 85/85 vs C 78/83;rai 83/82 vs A 74/74);D 相对 base 在 rai 是持平、在 mike 是 +3~+7。
- mike:SAD-tc s0 pass-10 = 85(与 D 同水平);LE50 s1 token 对齐 = 77(持平,过滤无增益);ADDALL s0 低端点 68。
- rai_Cs2t96490 / rai_ces2p10 首次因 vLLM 引擎启动失败(GPU1 短暂被占?)未评,已重跑(链 #6)。
- 待 mike 上 A(纯 CE)三 seed 高端点落地后给四格判断:关键是 A 在 mike 读多少(若 A≈78–80,则 D/SAD-tc 的 85 是方法效应;若 A 也 85,是平台读数)。
...
- 20:30–21:05 mike 连续拒绝登录("logins exceed limit",与后台进程数或残留会话有关,外部无法清理);**smic 共享同一 /ddnA/work**,
  ...
  (原版 SmartAD seed2 需人工提交评测)。
- 端点同步循环改经 smic(pid 2957480);评测结果轮询改经 smic(b20x03ojk,15 分钟)。
- 21:05 快照:已评 7 格(base 78/82;低端点 B s1 73、C s1 68、D s2 64、LE50 s1 70、SADTC s1 p3 66),8 格在评,25 已提交;
  96,490 端点已训完 9 个(D ×3、C s1/s2、A ×3 参照、B s2),SAD-tc s2 pass-10。
...
- **偏离预注册的 1:1 混合**:1:1 会把 D0 压到 442–2,285 token(与 REPAIR 等量),训练集退化;改为 **D0 + 该 seed 全部可用后缀**
  (10,091 / 10,346 / 11,934 token),ADD-DEMO 臂同理保留 D0+全部 ADD(16,099)并另有 1:1(12,900)版;token 端点 [28,947/96,490] 统一。
  两臂"等追加预算"体现在采购上限相同(1.5M),实际产出不同即是结果的一部分。配置 `pi1_alfworld_k32_d0_plus_repair_all_s{0,1,2}_tok.yaml`(taskeq 树,preflight 过)。
- 表 §U3。下一步:三个 REPAIR 臂投 mike,eval 循环加 `d0repall_v2`。
...
- REPAIR s1(10 请求)/ s2(13 请求)在 P1 上每个请求首个调用即失败(全部 'reserved' 悬置,各 ~2.2k prompt + 2,048 预留),0 可用;
  ADD-DEMO 末尾的 18 条悬置预留也是 P1 耗尽所致。失败目录移入 `_trash/`;**以 AZURE_SLOT=P2 重跑 s1/s2**(链 logs/rai_chain_repair_s12_p2.log)。
- 用户要求"三个 API 用完了跟我说":P1 已用尽,已告知。

### ADD-DEMO 采集完成:20 条可用 demo,结算 639,611 token;成本口径更正(2026-09-22 18:10 CDT)
- ADD-DEMO(32 任务,attempt 3–5,T=0.7,每任务 1 条目标):824 次请求(806 reported,**18 条悬置预留 = 78,677 token 上界**),**结算 639,611 token**;
  23 条验证成功、30 次失败尝试;扫描过滤去掉 3 → **可用 20 demo / 249 回合 / 6,450 token**(D0:30 / 424 / 9,649)。上限 1.5M 未触及(尝试数用完)。
- **成本统计更正**:我给用户报的 REPAIR "1,408,383 token"与中途的"813k"都是把 usage.jsonl 每行(预留+上报+中间状态)累加的结果——错误;
  正确口径(每请求取最后状态、只算 reported)REPAIR s0 = **212,804**。已改表 §U 并向用户更正。**规则:成本永远用 alfworld_cost 的口径算,不手写累加。**
- 下一步:并集银行 D0+ADD 1:1(token)与 D0+全部 ADD 两种,token 端点 [28,947/96,490](与 A 同更新次数),三 seed 投 mike;REPAIR 臂不训(退化),写进报告。
...
- 成本 **更正(18:10)**:我第一次把账本每一行(含预留、中间状态)都加了 → 1.41M 是错的;按"每请求取最后状态、只算 reported"(成本模块口径):**248 次请求,212,804 token 结算,0 悬置**(1.5M 上限没碰到,自然结束)。可用银行 **2 demo / 17 回合 / 442 token**;D0+REPAIR 1:1 并集 = 884 token(退化,不能训)。
- **读法**:教师从学生卡住的状态出发也很少能救回(2/11),每次尝试 ~19k token;限制是材料产出率不是预算;REPAIR 在这个 setting 的材料产出率极低。
  这与 09-16 HotpotQA 的教师接管 12/24 同向但更差(ALFWorld 剩余步数受 40 步上限挤压)。
- 处置:① 不训 884 token 的并集;② seed1/2 的 REPAIR 采集排在 ADD-DEMO 之后跑(链 pid 见 logs/rai_chain_repair_s12.log,各 1.5M 上限)看 2/11 是否典型;
  ③ ADD-DEMO(1.35M,15 任务)按计划训;④ 报告里正面写"学生状态纠错在 ALFWorld 40 步内基本采不到"。表 §U。

### rai 选例复现 32/32;GPU1 被同事占用;mike 9 跑 + 坏节点重提(2026-09-22 16:33 CDT,时钟核对后改)
...
|---|---|---|---|---|
| base 重复 | 81 | −2.14 | 11(7/4) | 0.55 |
| **CE 10 遍 s0 / s1** | **74 / 74** | −7.14 / −7.14 | 22(16/6) ×2 | 0.052 |
| 高扫描半 10 遍 s0 / s1 | 76 / 77 | −5.7 / −5.0 | 26 / 25 | 0.17 / 0.23 |
| 过滤 D0 10 遍(85 步) s0 / s1 | 69 / 72 | −10.7 / −8.6 | 29 / 26 | 0.008 / 0.029 |
- **base 跨平台稳定**(LONI 均值 81.0 vs rai 82.5),**训练后模型在 rai 少约 5 题**(同一合并权重;vLLM 0.27.1 vs 0.29.0)。
  rai 内部:过滤 D0 对 CE −5/−2,高扫描对 CE +2/+3,都在 rai 自身重复噪声(11 题翻转)内 → **过滤材料检验在此剂量下不可判**。
- **结论/处置**:① 主比较必须回到单一评测平台——明天 hpg 恢复后用同一 vLLM 版本把 base、A/B/C/D、REPAIR/ADD-DEMO、忠实基线全部重评;
  ② 在此之前 rai/mike 上的评测只作同平台内相对读数并标注平台;③ token 对齐的过滤 D0(mike le50tok)出来再看材料问题。
- GPU4 链继续:三 seed support rollouts 开始(16:03)→ REPAIR s0 采集自动接。表 §T。
...
- 两个 caveat:① rai 平台单次 base(84;LONI 三次 79/84/80),同平台 CE 10 遍参照 `rai_ces0p10 / rai_ces1p10` 已插到 GPU4 链最前;
  ② 该臂 10 遍只有 85 步(44k token),落在 D0 3 遍坑(55 步)与 10 遍恢复(184 步)之间——**剂量位置混杂**。
  已注册 `pi1_alfworld_k32_le50_tok.yaml`(同银行,token 端点 28,947/96,490 = 与 D0 同更新次数),mike 提交 le50tok s0/s1 860766/67(preflight 过)。
- 读法暂定:"只过滤材料"不足以赢 base;剂量对齐后再定。表 §S。
- GPU4 链 #2 重启(pid 2246098):CE s0/s1 10 遍 rai 评测 → 三 seed rollouts → B s0 → C s0。
...
C = 87 条示范普通 CE、D = 87 条任务等权 CE;同一 base、**匹配实际监督 token 与更新次数**、三 seed;各银行教师成本分列;
④ ADD-DEMO / REPAIR 同步准备:冻结同一学生起点,K=32 内 rollout + 状态清单;两臂同追加预算、同训练规则、先用普通 CE;
⑤ 并行核实 SmartAD 原始 NLL/分段损失与 Kang FTP 生成流程,修正偏离;冻结可重复评测配置;不用 base 三次波动定义"4pp 以下无效";
⑥ 决策规则:B>A 且 D>C → 支持任务等权;C>A 而 B 无改善 → 收益来自材料;REPAIR > 等预算 ADD-DEMO → 支持学生状态纠错;都无 → 停。
看整体成功率、三 seed、逐题配对;clean/cool 作机制证据;候选冻结后与三条忠实基线一起进 unseen 确认。用户 13:32:"利用我们新的 cluster 并行快速解决"。
**执行**:
- **Codex#15**(新 worktree `tc-alignment-taskeq`,分支 `alf-taskeq` 自 alf-baselines):新方法 `pi1_ce_taskeq`(每次更新内各任务等权:
  w_t = (N/n_tasks)/T_t,总权重 = token 均值),`exposure_tokens: [28947, 96490]` 端点(D0 上与 3/10 遍同步),配置 B/C/D,CPU 测试。
- Codex#14(REPAIR/ADD-DEMO 管线)继续。基线核查 Codex#16 待 #14 结束后派。
- LSU 集群(mike/smic):密码登录被 sshpass 卡住(疑似 Duo 二次验证),已停并销毁密码文件;请用户在 Mac 上 `ssh-copy-id` 装 rai 公钥。
- rai 全线索引:GPU1 SmartAD s2(~20:00);GPU4 评测链(~16:40)→ 之后改跑 B/C/D(all87 链视 #15 就绪与否取消)。hpg 明日。

### 用户解除教师预算限制;REPAIR-CE 管线开工(2026-09-22 13:19 CDT)
...
- mike LE50 s2 高端点 74 → **过滤 D0 三种子 76/77/74 = 75.7**(A 77.7):过滤本身无增益。le50 s2 训完(06:1x CDT),GPU3 链 v3 开始 1min s0。
- **rai 五模型三种子表出齐(tables §T11)**:CE 75.0 / SmartAD 原版 81.3 / SAD 原版 76.0 / Kang 81/70/79 = 76.7 / C 82.3 / D 81.0 vs base 82.5——rai 上没有方法到 base;排序与 mike 一致(除 Kang)。链 #7 结束(06:08 CDT),链 #8 开始评 ALT1 s0。
...
- **mike ALT1 s1 = 87 → ALT1 三种子 mike 86/87/87 = 86.7**(A +9.0、base +5.3、原版 SmartAD +4.0、修正版 +0.3);rai 88/87。
...
- **hpg**:A 80/76/78 = 78.0(base 81.0);**ALT1 85/85/88 = 86.0**(A +8.0,base +5.0);原版 SmartAD 82/80/81 = 81.0;原版 SAD 69/82/76 = 75.7;Kang 78/74/85 = 79.0;C s0 85 → 身份效应在第三个平台成立,ALT1 高出原版 SmartAD 5.0、原版 SAD 10.3。
...
- rai PER2 s1 = 80(A s1 74);hpg 第二批 25 格已提(43096243–43096267,队列 38)。
- **rai ALT1 s2 = 86 → ALT1 三平台三 seed:mike 86/87/87 = 86.7、rai 88/87/86 = 87.0、hpg 85/85/88 = 86.0;A 分别 77.7 / 75.0 / 78.0。**
- mike PER2 s0 = 83(rai 89/84)。
- rai PER2 s2 = 83 → PER2 rai 89(84)/80/83,mike 83/–/86:2 条/任务 ≈ 1 条/任务偏低 2–3 题,不高于 ALT1。1min s0 训练到 184/184(10:54 CDT),链 #8 随即在 rai 评 ALT1MIN s0(≈11:20);链 #10 开始训 1min s2。
- mike PER2 s1 = 82 → **PER2 三种子 mike 83/82/86 = 83.7**(A +6.0,ALT1 −3.0);rai 84.0。链 #8 于 10:50 CDT 开始评 ALT1MIN s0。
- 待定:1min s0(rai 评测中)/s1/s2;若 s1 与 rai 同向,材料身份就是今晚最强的方法因子,需要解释"哪些轨迹好"(18 个命令序列不同的任务 + thought 文本差异)。
...
- 16:06 CDT **第二个结构性限制,已请用户选 A/B**:第一天判定 = 6 个候选集的 cv 分与最终成绩的排序相关。
  精确置换分布下单侧 p≤0.05 所需 Spearman:n=6 **≥0.829**、n=7 ≥0.714、n=8 **≥0.643**。叠加每集 cv 分约 2.7 任务的抽样噪声,
  **中等强度的真实关系在 n=6 下也很可能读成"不显著"**。
  选项 A = 保持 6 个(今晚出,只能读"方向是否一致");B = 加 rnd4/rnd5 → n=8(+8 cv 训练 +4 最终,抢 hpg,多半过午夜)。
  **默认 A,等用户回复**。**B 已预备好**:rnd4/rnd5 包列表在 scratchpad `rndx/`,
...
- 16:11 CDT **调度器加固并重启**:原先的"adapter_config.json 存在即拉"会和 hpg 上仍在写大权重文件的训练撞车;
...
- 16:11 CDT **MIX1 s0 训完**(16:10,184/184,`results/all87_mix1_v2/seed-0/tokens-96490/lora`),链 #8(pid 4150908,每 10 分钟轮询)会在 GPU4 自动评。
...
- 18:15 CDT **MIX1 s1 = 90/140(rai)** → MIX1 两 seed 均值 **88.0**,ALT1 87.0(88/87/86)。
  **按字面阈值"≥ 88"刚好达到,但机制证据不支持**:heat 6/4 → 均值 **5.0 = ALT1 的 5.0**,换进来的 heat 轨迹没有让 heat 变好;
  s1 多出来的局在 look 11 / cool 10 / two 18——**这些类型的材料与 ALT1 完全相同**,所以差异是 seed 噪声(ALT1 自己三个 seed 跨 86–88,1min 跨 77–85)。
  **结论**:轨迹质量不可按类型拆分拼接;MIX1 诊断关闭。链 v3 已退出 → GPU2 空,归 rai 第二平台评测调度器。
- 19:36 CDT **首个 cv adapter 已拉回**:cv_1alt_f0 于 19:34 拉到 rai,sha 76fb0f77… 与 hpg 一致。
  **⚠ 我 16:11 那次改调度器引入的 bug**:为了让 `.launched` 标记一定落地,我在启动打分前先 `mkdir -p $run`,
  但 `score_support_rai.sh` 第 14 行**拒绝已存在的 run 目录**(exit 2)→ 第一次启动 19:34 即失败,没有打分。
  **修法**:标记移到 `data/cv_score/.state/<name>/`,run 目录只由打分脚本创建(baselines 提交见 git log);只含标记的残目录移入 `_trash/`;
  调度器重启(`CV_GPUS="1 3"`),19:35:44 重新启动 cv_1alt_f0 打分,合并已开始。**损失约 1.5 分钟**。
  观察器同步改为看 `.state` 下的标记并重启(bmz8y66uh)。第二平台评测调度器的标记本来就在 run 目录之外,不受影响。
- 19:52 CDT **✅ 打分链端到端打通**:cv_1alt_f0 19:52 打完(合并 2 分 + 起服务 4 分 + 32 题 rollout 9 分 ≈ 16 分钟/个)。
...
- 23:42 CDT **✅ 第一个完整候选集:rnd1 cv = 19/32**(折 7/3/4/5;在训练过的题上 17/17/16/16 of 24;留出平均 21.56 步)。rnd2 f0–f2 已拉回,GPU3 依次打。
- 23:42 CDT **hpg 第二个自造瓶颈:部门 QOS cpu=128**(QOSGrpCpuLimit;部门用 124/128,我们 4×14=56)。rai 实测同一训练进程 **1.0 核**。
  试验(组 burst QOS 上的 3 分钟 CPU 小作业):只改 `NumCPUs=4` → 启动时仍分到 14 核(`--cpus-per-task` 说了算);
  改 `CpusPerTask=4`(+ `MinCPUsNode=4`)→ 实际分到 4 核(nproc=4)。**已对 24 个排队训练设 CpusPerTask=4、NumCPUs=4、MinCPUsNode=4**;
  评测保持 14 核(14 个游戏客户端)。`train_b200.slurm` 默认改为 `--cpus-per-task=4`。
  **排队现在是 32G / 4 核**:一个 14 核作业结束可放进 3 个我们的训练;之后受部门 GPU 上限 24(已用 ~16)与物理节点限制。
- 23:42 CDT rai RT:rnd1 148/184、rnd2 150/184 步(~00:20 训完,~00:35 出分)。
- 00:35 CDT **rai 训练的 rnd2 s0 = 87/140**(随机集!与 ALT1 rai 88/87/86 同级;分类型 heat 9/16 远高于 ALT1 的 5,cool 5/25 与 1min 同低;对 ALT1 s0 配对 10 vs 11,p=1.0)。
  rnd1 RT 在 GPU2 评测中;GPU4 链已转入 nll1 训练(00:32 起,~05:00 训完)。
- 00:35 CDT **hpg 现在 12 个作业在跑**(降 CPU 后;4 核作业 64.0–64.5 s/step,与 14 核 63.7 相同)。估计:rnd3 折 ~02:00–03:00,1alt f1–3 / 1min ~03:25–03:50,
  剩余 15 个(nll1 cv、全部 fin)~02:00 起 → ~05:20 训完 → 判读 ~06:00 CDT。
- 00:35 CDT **cv 打分加一张共享卡**:调度器 pid 1008222,`CV_SHARED="4:0.45:50000:<04:40 CDT>"`——GPU4 在我们 nll1 RT 训练(CPU 瓶颈)旁边以 vLLM 0.45 打分,
  **仅当我们的 alf_pi1_train.py 在卡上、空闲 ≥ 50 GB、且早于 04:40**(训练约 05:00 结束后链要起 0.80 的评测,不能撞)。GPU4 与 GPU1 同型号(RTX PRO 6000 Blackwell)。
  **注意**:cv 打分的硬件本来就混用了 GPU1(Blackwell)与 GPU3(A100)——单检查点跨硬件可差 11 局(1min s2),这是 cv 分的一个噪声源,判读时说明。
- 00:43 CDT **rai 训练的 rnd1 s0 = 88/140**。rai 训练、rai 评测的 seed-0 最终成绩:**ALT1 88、rnd1 88、rnd2 87、1min 77**(rnd3 / nll1 ~05:30)。
  **读法(Q1 的早期信号,单 seed)**:两个随机集与 ALT1 同级 → **"选哪条轨迹"对最终成绩的影响可能小于 seed 噪声**
  (ALT1 自己 86–88,1min 77/85/83)。若 rnd3 与 hpg 训练的 rnd 最终成绩也如此,则按 §3.1 登记的读法:在这个银行里材料选择几乎不影响结果,
  任何选择信号(包括 cv 分)都无从体现——是一个独立的结论,需要单独说,不能读成"cv 方法失败"。
  分类型:各集总分相近但构成不同(heat:ALT1 5、rnd1 4、rnd2 9;clean:rnd1 21;cool:ALT1 9、rnd1 7、rnd2 5)。
  **对"打败基线"的含义**:原版基线的差距看来来自配方(pi1 CE、1 条/任务、96,490 token 终点),而不是挑哪条轨迹。
- 00:52 CDT **用户:"先等更明显的信号再总结成文档"** → 不写总结文档。我的默认(已告知,可撤):28,947 端点的 cv 打分挂**最低优先级**。
  动机:同一材料 28,947 端点 62–76(ALT1 71/76/67)vs 96,490 端点 86–88,剂量差 ~15 局 = 材料差的 3–5 倍;cv 分能否看出这 15 局是 cv 信号是否有用的**必过检验**,
  也是"用跨任务学习收益定剂量"这一方向(已向用户提出,待定)的首个免费检验。
- 00:52 CDT **调度器改为 `tools/cv_score_dispatch_ep.sh`(新文件;不改正在运行的脚本)**:
  主 **pid 1080551**(96,490,`CV_GPUS="1 3"`,`CV_SHARED="4:0.45:50000:04:40"`,日志 logs/cv_score_dispatch.log);
  低剂量 **pid 1080580**(`CV_ENDPOINT=28947 CV_YIELD=1`,根目录 `data/cv_adapters_t28947`、`data/cv_score_t28947`,日志 logs/cv_score_dispatch_t28947.log)。
  新增 `scorer_on`:任何实例的 `score_support_rai.sh` 在跑(含合并的 CPU 窗口)即视该卡为占用,防两个实例抢同一张卡。
  低剂量实例 00:51 已在 GPU4(共享,0.45)打 cv_1alt_f0 的 28,947。
  **⚠ 我的操作失误**:按 argv 精确匹配停旧调度器,**连带杀了 cv_rnd2_f3 的打分子 shell**(子 shell 与父进程 argv 相同);打分进程本身成了孤儿仍在跑,
...
- 02:08 CDT **论文 Figure 1**(tc-paper 91f54fa):11 个 ALFWorld 训练集在 mike 上 3 遍 vs 10 遍——3 遍全部低于 base(低 5.6–12.4 pp),10 遍回升 3.6–13.6 pp;引言数字据此写实。
- 02:08 CDT **RQ3 扩展到 14 个配置(7 选材 × 2 剂量)**:给所有最终训练的 28,947 端点也挂了评测(标签 hpg_<TAG>LO_s<k>,43168334–55);
  `cv_verdict` 预登记主读数只用 96,490(跳过 LO);补充分析 `tools/cv_configs.py`(蒙特卡洛置换)。
- 02:08 CDT **⚠ 早期信号(单个候选)**:rnd1 的低剂量 cv = **18/32**,高剂量 19/32——留出分几乎分不出剂量(最终测试上剂量差 ~12 局 ≈ 32 题里 2.7 题,在噪声内);
  留出平均步数 23.3 vs 21.6、训练过的题 12.5 vs 16.5 of 24 差得更明显。等更多候选再读;若 cv 分确实看不出剂量崩塌,步数信号可能是更好的选择统计量。
- 02:08 CDT HotpotQA:Codex #5 base 模式交付并提交(hqk32 9f0a10bd,27 测试),**base 评测 43168458**;Codex #6(折训练 + 支持题打分,剂量 1/3/5/10 遍)进行中。
- 02:10 CDT **TVD 的平手规则改为"留出平均步数更少者优先(失败记 40 步),再按教师 token 少"**(原为"剂量小者优先")。
...
- 03:36 CDT **rai 打分是方法判读的瓶颈**(剩 ~15 个高剂量折 + 同量低剂量折,每个 ~16 分钟,GPU3 + 04:40 前的共享 GPU4)→
  `tools/regpu_after_gpu2_chain.sh` 等 GPU2 的 RT 链(618765,~05:40)退出后:只停**顶层**调度器(PPID 不是调度器的那个;试跑确认不碰打分子 shell),
  以 `CV_GPUS="1 2 3"` 重启 4 个 cv 调度器实例,**第二平台评测调度器暂停**(最低价值,标记在,随时可续)。GPU4 05:20 后归 unseen 列。
- 04:09 CDT **1alt cv = 21/32**(8/5/4/4;训练过的题 17/19/17/16;留出步数 **20.09**,目前最低)。96,490:rnd1 19 / rnd2 20 / rnd3 21 / 1alt 21 → 按新平手规则(步数)TVD 暂选 1alt@10 遍。
  已知最终(rai 训练 s0):1alt 88 / rnd1 88 / rnd2 87;hpg 最终待出。
- 04:54 CDT **1min cv = 23/32(最高)**(7/5/5/6;训练过的题 14/17/16/17;步数 20.28)。96,490 排序:1min 23 > 1alt 21 = rnd3 21 > rnd2 20 > rnd1 19。
  **⚠ 与最终成绩不同向**:1min 的最终是候选里最弱的(rai 77/85/83 = 81.7;hpg 79/85/72 = 78.7),1alt / rnd1 / rnd2 约 86–88 → 按成功数的 cv 会挑中最差的材料。
  这正是事先写明的分辨率风险(32 题里差 2 题在 SD≈2.7 以内)。剂量维度 cv 分得开(rnd2 20→16、rnd1 19→18,步数一致变差)。
  **含义(待 nll1 / pool / hpg 最终落地再定)**:选材信号可能是噪声,TVD 的价值集中在剂量;若 pool 的 cv 最高,TVD = 全池 SFT。
- 04:54 CDT 时间更正:RT 链 nll1(GPU4)01:32 起训、rnd3(GPU2)01:43 起训,各 ~4.5 h → **~06:05 / ~06:15 训完**;unseen 列约 06:20 起(不是 05:20),GPU2 交接约 06:30。
- 04:55 CDT **提议(未采用,待用户定)——TVD 的"分解式选择"**:剂量对所有选材共享 → 用**所有选材的 cv 合并**来选剂量(5 个选材 ≈ 160 个留出结果,而不是 32 个),
  再在该剂量下选"全池 vs 每任务一条"。动机 = 事先写明的分辨率问题(每个配置只有 32 个留出结果,选材差异 1–2 题不可分辨)。
  **诚实性**:这是在看到 1min 的最终成绩之后提出的,若采用必须在论文里写明"逐配置 argmax 是事先规则,分解式是看到分辨率失败后的修订",
  并用 HotpotQA(只有剂量维度)和新候选池做事后验证。**论文里的方法规则今晚不改**,早上交用户决定。
- 05:29 CDT **hpg 训练的 rnd1 s0 = 85/140(hpg 评)**(rai 训练 rai 评的 rnd1 s0 = 88)。搬运器已按新优先级把评测挪到组账户(43123104、43123105)。nll1 cv 折 0 = 7/8。
- 05:33 CDT **nll1(SmartAD 最低 NLL 选材)rai 训练 s0 = 79/140**。选材平均 token-NLL:nll1 **1.778** → 79;1alt 1.942 → 88;rnd1 1.910 → 88;rnd2 1.922 → 87;1min 1.911 → 77。
  **按拟合度(NLL)挑材料反而更差**(比随机选材低 ~8.5 局);其余集合 NLL 与最终无关。这是论文"拟合代理不预测迁移"的直接证据(单 seed,hpg 最终待出)。
  rai GPU4 的 RT 链 05:31 退出 → **unseen 列已开跑**(先 base 冒烟)。
- 05:44 CDT **rnd3 rai 训练 s0 = 77/140** → rai 训练 s0 最终:1alt 88 / rnd1 88 / rnd2 87 / nll1 79 / 1min 77 / rnd3 77。
  **更正 05:35 的说法**:随机选材本身就跨 77–88(与单个选材的 seed 散布同量级:1min 77/85/83),nll1 的 79 **在随机范围内**,
  只能说"按 NLL 选不比随机好",**不能说"更差"**。论文引言已改为"no better than"。
  cv(10 遍)vs 最终(rai s0):1alt 21→88、1min 23→77、rnd1 19→88、rnd2 20→87、rnd3 21→77 → 目前**负相关**;选材维度 cv 无信号,和 seed 噪声分不开。
  GPU2 05:40 交给 cv 打分(4 个调度器实例以 GPUs 1 2 3 重启,第二平台调度器暂停);unseen base 冒烟 108/134 局进行中。
- 05:45 CDT 选材维度无信号、剂量维度有信号 → **剂量曲线变成方法的核心证据**:两个 20 遍剂量训练恢复 Nice=0(部门队列下一批;~6.7 h,约中午出)。
- 05:50 CDT **unseen 冒烟过:rai_UNS_base = 73/134(54.5%)**(同一 base 在 hpg 上 69/134 → 跨架构差 4 局,所以整列只用 rai GPU4)。链继续评 A / C / SMORIG / SADORIG / KANG × 3(~10 分钟一个,约 08:30 完)。
- 05:52 CDT **hpg 训练 rnd1:s0 85 / s1 87 = 86.0**(hpg 评)。观察器改为批量:失败 / 候选集 4 折凑齐 / hpg 连不上 立即唤醒,普通评测完成攒 45 分钟一报(bk14v4bj6)。
- 06:31 CDT **nll1 cv = 22/32**(7/4/5/6;步数 20.69)。96,490:1min 23 > nll1 22 > 1alt 21 = rnd3 21 > rnd2 20 > rnd1 19(pool 待出)。
  **低剂量 cv**:1alt 19(<21)、rnd1 18(<19)、rnd2 16(<20)→ 三个候选都是 10 遍 > 3 遍,**剂量方向 cv 全对**,但低估了差距(1–4 题 vs 预期 ~4.7 题)。
- 06:31 CDT **hpg 低剂量最终:rnd1 3 遍 = 66 / 65 = 65.5 vs 10 遍 86.0 → −20.5 局**(hpg 上的崩塌比 mike 更大)。
- 06:31 CDT **unseen(rai GPU4)**:base 73,A(SFT 首条)s0 65 / s1 66 → 纯 CE 在 unseen 上也低于 base。
- 07:19 CDT **hpg 训练 / hpg 评测的最终**:rnd1 85/87 = 86.0(3 遍 66/65);**rnd2 76/75 = 75.5**(3 遍 69/71);rnd3 s0 82;nll1 s0 81。
  **⚠ 训练平台 / seed 的影响很大**:同一 rnd2 选材,rai 训练 s0 = 87,hpg 训练 s0/s1 = 76/75(差 ~11 局),大于选材之间的差异。
  cv(10 遍)vs hpg 最终:rnd1 19→86.0、rnd2 20→75.5、rnd3 21→82、nll1 22→81(1alt / 1min 的 HT 待出)。
  **低剂量 cv**:1min 18(<23)→ 四个候选全部 3 遍 < 10 遍;最终上 3 遍全面崩(rnd1 −20.5、rnd2 −5.5)。
- 07:19 CDT **unseen(rai GPU4)**:base 73;SFT 首条(A)65/66/67 = 66.0;**SFT 全池(C)77/77**(高于 base)。继续评 C s2、SMORIG、SADORIG、KANG。
- 07:19 CDT hpg:K=8/16 曲线已开跑(k8/k16 共 11 个在跑)、两个剂量训练已开跑。
- 08:08 CDT **✅ 预登记的第一天判读 = NO_RELATION(选材维度)**(docs/method/2026-09-24_day1_verdict.{txt,json}):
  6 个候选,hpg 训练 / hpg 评测,s0/s1:1min cv 23 → 77.0;nll1 22 → 82.5;1alt 21 → 86.5;rnd3 21 → 81.5;rnd2 20 → 75.5;rnd1 19 → 86.0。
  Spearman **−0.174**(精确置换 p = 0.64);cv 选中 1min,最终最差,遗憾 9.5 局。随机集之间 SD 5.27 > seed SD 3.27(材料确实有影响,但 cv 抓不到)。
  补充:留出步数 +0.486(p = 0.18);**四折一致性 Kendall W = 0.164(p = 0.55)→ 各折各选各的**;**零成本对手"银行更短"+0.714(p = 0.068)**
  ——固定 token 预算下更短 = 每条示范更多遍 → 又是剂量。
  **含 3 遍的配置分析(8 个配置):Spearman +0.491(p = 0.11)**,正相关全部来自剂量维度。
  **按用户的第一天决策规则:停止扩展"用 cv 选材料"**;按已完成的对照,杠杆是**剂量(每条示范的遍数)**。剂量曲线(3–20 遍)约 12:30–13:00 出。
- 08:08 CDT unseen:A 65/66/67、C 77/77/74、SMORIG 70/70(s2 在评)。
- 08:39 CDT **早上汇总已发**(Discord 1552675840230293634,附 docs/reports/2026-09-24_overnight_summary_ZH.md)。
...
- 09:21 CDT **用户新计划(附件)**:停止"CV 选材料";"选训练量"只做一次限时验证,论文还不改成剂量方法。三个任务:
  ① 最高优先:K=8、K=16 普通 CE 补训到 96,490 监督 token,2 个 seed,记录实际遍数与更新数(正式 baseline 不变);
  ② 剂量曲线加全池 SFT,横轴 = 累计监督 token,候选含未训练 base;比较四种策略(固定现行 / 固定最大 / 各 K 统一 token / 支持集 CV 选点);
     只有曲线显示不同 K / 环境的最优量不同,才加**一个**固定材料(全池)的 4 折剂量 CV,CV 与最终同一 token 坐标,平分规则事先写死;
  ③ HotpotQA 作确认环境,看结果前冻结规则;Kang 采购批准;SmartAD 按原方法补多候选数据;报告 K=32 只有 19 题有示范。
  论文:保留负结果,撤掉确定性说法,剂量写成"待检验的问题",删掉"零额外训练",承认 CV 早停久已有研究(JMLR v23 21-0983)。约 12 小时后冻结方向。
- 09:23 CDT **HotpotQA 剂量规则预登记**(docs/method/2026-09-24_dose_selection_prereg_ZH.md,79de4480):T = {0, 2,465, 7,395, 12,325, 24,650, 49,300}
  (0/1/3/5/10/20 遍全银行);cv(t) = 四折留出 EM 之和;t* = argmax,平分取更少 token;t* = 0 → 报告"无适配收益";
  CV 只有比策略 1–2 中较好者高出 dev-500 配对 SE 以上才算有价值。
- 09:24 CDT **小 K 训练量对照开跑**:rai GPU2 = K8 CE s0,GPU3 = K16 CE s0(tools/rai_chain_smallk_dose.sh,taskeq 配置
  pi1_alfworld_k{8,16}_ce_tok.yaml;K8 端点 13,990/28,947/48,245/96,490 = 10.0/20.8/34.7/69.2 遍,27/56/93/185 步;
  K16 端点 28,947/34,970/48,245/96,490 = 8.4/10/13.9/27.6 遍,56/67/93/184 步;训完同卡评 140 局,标签 rai_SMALLK{K}CE_t{tok}_s0)。
  hpg:K8 s1 43190780(评测 81–84)、K16 s1 43190785(评测 86–89)、**全池剂量 s0 43190790**(端点 28,947/48,245/67,543/96,490/144,735/192,980 =
  0.87…5.78 遍,55…367 步;评测 43190791–96)。
- 09:25 CDT HotpotQA 按遍数的 fold / 剂量作业**取消**(43169315/21/27/33、剂量 43169339 及其依赖)——用户要求 token 轴;
...
- 15:19 CDT **ALT1 剂量曲线完整(hpg 训练 / hpg 评,seed 0)**:28,947 → 73、48,245 → 78、67,543 → 83、96,490 → 82、**144,735 → 92、192,980 → 92**
  (2.6 / 4.3 / 6.0 / 8.6 / 12.8 / 17.1 遍)。**比现行 10 遍(82)多 10 局,比 base 81.0 高 11 局**;12.8 遍后持平。单 seed。
  → 按预登记:曲线在范围内基本单调上升 → 先采用固定(更大)训练量策略;等 rnd1 与全池曲线确认。
- 15:23 CDT **用户:先停一下,不赶 ICLR,转投 ICML,争取 best paper 级别,把工作做扎实。** → 不再提交新实验、不再为赶稿改论文;
  已提议让已提交的 hpg(7 跑 + 97 排队)与 rai 4 条链跑完(都是 ICML 版同样需要的数据),跑完出一份"已知 / 缺什么 / ICML 需要补什么"文档;
  等用户确认是否也要停掉排队/在跑的作业。
- 16:00 CDT rai 内评(辅助,非论文列):**K16 s0**(GPU3 A100)28,947 → 67、48,245 → 63、96,490 → 70;**K8 s0**(GPU2 Ada)13,990(10 遍)→ 76、28,947 → 71
  (rai base 82.5)。K8 s0 已同步到 hpg、B200 评测已提交(hpg_eval_jobs.txt)。GPU3 空出——按用户暂停指示不再提交新作业。
- 16:56 CDT **rnd1 剂量曲线完整(hpg,seed 0)**:28,947 → 66、48,245 → 70、67,543 → 83、96,490 → 87、**144,735 → 92、192,980 → 92**。
  **两条独立的一题一条选材(ALT1、rnd1)形状相同:单调上升,约 13 遍起在 92/140(65.7%)持平**,比 base 81.0 高 11 局,比 10 遍默认高 5–10 局。
  → "默认配方训练量不足"的证据加强;按预登记,单调上升 → 固定更大训练量策略。全池曲线(hpg s0 ~18:10,rai s1 ~20:00)待出。
- 17:40 CDT **小 K 对照(部分)**:hpg B200 —— K16 s1(hpg 训)28,947 → 69、34,970(10 遍)→ 62、48,245 → 67、**96,490 → 76**;K16 s0(rai 训)28,947 → 65(其余在评)。
  rai 内评 K8 s0 13,990(10 遍)→ 76、28,947 → 71、48,245 → 68、96,490 → 77;K8 s1 76/71/71(96,490 在评)。
  初读:K16 补训练量 62 → 76(+14)但仍低于 base 81.0;K8 补训练量没有增益(~71–77)→ 小 K 下任务覆盖仍然重要。等 K8 的 B200 数与 K16 s0 齐。
...
- 18:56 CDT **HotpotQA 确认链四折齐,按冻结规则判读**(docs/method/2026-09-24_dose_selection_prereg_ZH.md):
  cv(t)= 四折留出 EM 之和:0 → 15、2,465 → 14、7,395 → 16、12,325 → 15、24,650 → 17、**49,300 → 18** → **t\* = 49,300 = 网格最大值(20 遍)**。
...
- 19:32 CDT **HotpotQA 确认链最终判读(预登记)**:全 K 剂量 s0 dev-500 EM —— 12,325 → 0.388、**24,650(固定默认)→ 0.414**、**49,300(固定最大 = CV 选点)→ 0.414**
  (2,465 / 7,395 待出,不影响判读)。**CV 选择 = 固定最大 = 0.414,与固定默认持平,差 0.0 < 配对 SE → 按预登记:CV 在 HotpotQA 上没有额外价值,采用最简单的固定训练量策略。**
  相对 base 38.0 为 +3.4 EM。留出 NLL 选 24,650 = 0.414;在-样本执行选 49,300 = 0.414 —— 三种选点信号在测试上都一样。
- 19:39 CDT HotpotQA 剂量 dev-500 曲线齐(s0):0 → 0.380、2,465 → 0.382、7,395 → 0.394、12,325 → 0.388、24,650 → 0.414、49,300 → 0.414(10 遍后持平)。
- 19:59 CDT **小 K 训练量对照判读(hpg B200 评测,2 个 run:s0 rai 训 + s1 hpg 训)**:
  K16:28,947 → 67.0、34,970(10 遍)→ 61.5、48,245 → 64.0、**96,490 → 72.0**(+10.5 但仍低于 base 81.0 九局);
  K8:13,990(10 遍)→ 76.0、28,947 → 72.5、48,245 → 68.5、**96,490 → 75.5**(没有增益)。
  → 按预登记:**小 K 补到 K=32 的训练量后仍明显较差 → 训练量不足不能充分解释小 K 的失败,任务覆盖 / 材料仍然重要**;而 K=32 加大训练量到 ~13 遍能到 91–92。
  **全池剂量(hpg 评)**:28,947 → 72.0(n=2)、48,245 → 72.5(n=2)、67,543 → 83、96,490 → 86、144,735 → 89、192,980 → 91(n=1,s0 其余点待出)→ 单调上升,到 5.8 遍仍在涨。
- 20:19 CDT **全池剂量曲线两个 seed 齐(hpg B200 评)**:28,947 / 48,245 / 67,543 / 96,490 / 144,735 / 192,980 ——
  s0(hpg 训)73 / 75 / 80 / **83 / 82 / 77**(96,490 后回落);s1(rai 训)71 / 70 / 83 / 86 / 89 / **91**(一直涨)。均值 72.0 / 72.5 / 81.5 / 84.5 / 85.5 / 84.0。
  ⚠ **高训练量处 seed 方差很大(192,980 处 77 vs 91,差 14 局)**:全池在 96,490 之后均值持平,"越训越好"只在单 seed 的 ALT1/rnd1 曲线上成立,需多 seed 确认;
  修正 19:59 那条"全池一直在涨"的说法(那时只有 s1 的高点)。
...
- 01:35 CDT(09-25)**ALFWorld 全池 4 折剂量 CV 打分完成(24/24,rai Blackwell)**:cv(/32)0 → 22、28,947 → 16、48,245 → 19、67,543 → 23、96,490 → 21、
  **144,735 → 25**、192,980 → 24(留出步数 22.3/25.0/23.6/20.7/22.3/18.6/18.2)→ **t\* = 144,735**(内部点)。
  测试(hpg,全池 2 seed 均值):144,735 → 85.5 vs 固定默认 96,490 → 84.5、固定最大 → 84.0 → **CV 比较好的固定策略多 1.0 局,远在噪声内**。
  cv 与测试的形状一致:7 个训练量点上 Spearman 0.79(p = 0.036)、Pearson 0.88 —— **CV 能读出剂量曲线的形状(低剂量先跌、后恢复),但在能区分的精度上不比固定策略好**。
  在-样本执行(Table 2)01:35 开跑。
- 02:27 CDT **ALFWorld 在-样本执行完成**(全池 rai s1 全量 checkpoint 跑 32 道训练任务):22 / 17 / 21 / 20 / 21 / 24 / **26** → 选 192,980。
  **Table 2 齐**:ALFWorld 固定默认 84.5、固定最大 84.0、留出 loss 84.0、在-样本 84.0、TVD 85.5;HotpotQA 全部 0.414。五种选点方式差距在噪声内。
  **所有实验跑完**;总结文档 docs/reports/2026-09-25_summary_0924_ZH.md 完成并发送。
