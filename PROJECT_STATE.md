...
- 成本 496 次调用 / **1,408,383 token**(上限 1.5M,stop_reason complete);可用银行 **2 demo / 17 回合 / 442 token**;D0+REPAIR 1:1 并集 = 884 token(退化,不能训)。
- **读法**:教师从学生卡住的状态出发也很少能救回(2/11),每次尝试 ~128k token;REPAIR 在这个 setting 的材料产出率极低。
